...
export async function handleSession(request, env) {
  ...
  if (!env.SESSION_SECRET || !env.PANEL_EMAIL || !env.PANEL_PASSWORD_HASH) return json({ error: 'El acceso no está configurado todavía. Avisa a Daniel.' }, 503);
  if (!turnstileConfigured(env) && !isLocalRequest(request) && env.PANEL_ALLOW_NO_TURNSTILE !== '1') return json({ error: 'Falta la verificación anti-bots (Turnstile). Avisa a Daniel.' }, 503);
  let body = {};
  try {
