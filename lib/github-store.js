...
async function describeFailure(response) {
  ...
  if (response.status === 401) return 'El acceso del panel a la web caducó. Avisa a quien administra la web: hay que renovar el token de GitHub en Vercel.';
  ...
  if (response.status === 403 || response.status === 404) return 'El panel no tiene permiso para editar la web. Avisa a quien administra la web: el token de GitHub de Vercel necesita acceso a este repositorio con permiso de lectura y escritura (Contents: Read and write).';
