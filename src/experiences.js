...
// The photo library comes from the CMS (content/home/gallery.json): index.njk
// serializes it into an inline <script type="application/json"> block so
// this module never has to hold (or be edited to update) the list itself.
// Each entry: { src, alt, altEs, big }. `big: true` forces that photo onto
// the tunnel's single largest frame slot (see the forced-placement block
// after `specs` is built) instead of leaving it to the random draw.
const IMAGES = (() => {
  try {
    return JSON.parse(document.getElementById('galleryData')?.textContent || '[]');
  } catch {
    return [];
  }
})();

// Alt text follows the active language, same as the rest of the page.
function imageAlt(image) {
  return getLang() === 'es' && image.altEs ? image.altEs : image.alt || '';
}

if (outer && stage && tunnel && IMAGES.length && !prefersReduced) {
  ...
  lightbox.innerHTML =
    '<figure><img alt=""><button type="button" class="experiences-lightbox-close">&times;</button></figure>';
  document.body.appendChild(lightbox);
  const lightboxImg = lightbox.querySelector('img');
  const lightboxClose = lightbox.querySelector('.experiences-lightbox-close');

  // The close button's accessible name comes from the CMS strings (rendered
  // as data attributes on the stage) and follows the active language.
  function setLightboxCloseLabel() {
    lightboxClose.setAttribute('aria-label', (getLang() === 'es' && stage.dataset.closeLabelEs) || stage.dataset.closeLabel || 'Close');
  }
  setLightboxCloseLabel();
  document.addEventListener('medusa:langchange', setLightboxCloseLabel);
  let lightboxOpen = false;

  function openLightbox(image) {
    lightboxImg.src = image.src;
    lightboxImg.alt = imageAlt(image);
  ...
  const setTunnelHeading = () => {
    textHeading.textContent = (getLang() === 'es' && stage.dataset.headingEs) || stage.dataset.heading || '';
  };
  setTunnelHeading();
