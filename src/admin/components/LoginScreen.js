...
export function LoginScreen({ onSubmit, busy, error, turnstileSiteKey, hint, compact = false, title = '', text = 'Entra con tu correo y tu contraseña. Cada cambio que guardes se publica solo en un minuto.' }) {
  ...
  return html`
    <div class=${`login ${compact ? 'login--compact' : ''}`}>
      <form class="login__card" method="post" action="#" aria-label=${title ? undefined : 'Entrar al panel'} onSubmit=${onSubmitHandler}>
        ${!compact && html`<img class="login__logo" src="/assets/Logo-negro.svg" alt="Medusa Show" />`}
        ${title && html`<h1 class="login__title">${title}</h1>`}
