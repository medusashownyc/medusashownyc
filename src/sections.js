...
// Each .ritual-thumb carries its "includes" list as JSON in data-includes
// (rendered by index.njk from content/home/packages.json): one entry per
// included format with its label in both languages plus, when the item is
// linked to a show in the CMS, that show's page URL and icon markup. Parsed
// once per thumb and cached on the element.
function ritualIncludes(thumb) {
  if (!thumb._includes) {
    try {
      thumb._includes = JSON.parse(thumb.dataset.includes || '[]');
    } catch {
      thumb._includes = [];
    }
  }
  return thumb._includes;
}
...
if (ritualFeature && ritualThumbs.length) {
  ...
  function applyRitual(index) {
    ...
    // (data-tag/-title/-desc/-alt = English, the -es-suffixed twin =
    // Spanish) — picking the suffix here is the only place that needs to
    // know which language is active; everything downstream just renders
    // whatever these end up holding.
    ...
    featureIncludes.innerHTML = ritualIncludes(thumb)
      .map((item) => {
        const label = `${item.icon || ''}${getLang() === 'es' && item.es ? item.es : item.en}`;
        return item.href ? `<li><a href="${item.href}">${label}</a></li>` : `<li>${label}</li>`;
      })
      .join('');
