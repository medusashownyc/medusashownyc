...
// Cards mirror the hero's .service-row list, which index.njk renders from
// the shows managed in the CMS (content/shows/*.json): every <li> carries
// its cover image (data-image), its crop preference (data-crop) and, via its
// <a>, the show page it links to. Nothing about a show is duplicated here.
//
// topBias 1 = keep the top of the photo fully in frame and crop only from
// the bottom (tall, tightly-framed portraits where the dancer sits near the
// top); 0.5 = crop evenly from both edges. The image's real width/height
// ratio is measured from the texture once it loads (see buildCard) instead
// of being hand-maintained per file, so a swapped photo can never distort.
const IMAGES = Array.from(document.querySelectorAll('.service-row li[data-service]')).map((li) => ({
  src: li.dataset.image,
  topBias: li.dataset.crop === 'top' ? 1 : 0.5,
...
if (container && IMAGES.length && !prefersNoWebGL()) {
  init();
}
  ...
  function endDrag(e) {
    ...
    if (dragMoved < CLICK_MAX_MOVE) {
      ...
      if (hit) {
        ...
        const url = IMAGES[hit.object.userData.imgIndex]?.url;
...
function buildCard(img, geometry, maskTexture, loader) {
  ...
  loader.load(img.src, (tex) => {
    ...
    if (ratio >= cardRatio) {
      tex.repeat.set(cardRatio / ratio, 1);
      tex.offset.set((1 - tex.repeat.x) / 2, 0);
    } else {
      tex.repeat.set(1, ratio / cardRatio);
